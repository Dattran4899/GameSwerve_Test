namespace MechSharingCode.Blueprints.BlueprintReader
{
    ...
    public abstract class GenericBlueprintReaderByRow<T1, T2> : BlueprintByRow<T1, T2>, IGenericBlueprintReader where T2 : class
    {
        ...
        {
            this.CleanUp();
            await using var csv = await CsvDataReader.CreateAsync(new StringReader(rawCsv), CsvHelper.CsvDataReaderOptions);
    ...
    public class BlueprintRecordReader<TRecord> where TRecord : class
    {
        ...
        private readonly List<CsvHelper.MemberInfo> fieldAndProperties;
        private          List<CsvHelper.MemberInfo> subBlueprintMemberInfos;
        ...
        public BlueprintRecordReader(Type blueprintType)
        {
            ...
            this.fieldAndProperties = new List<CsvHelper.MemberInfo>();
            this.Setup();
        }
        ...
        private void Setup()
        {
            ...
            foreach (var memberInfo in memberInfos)
            {
                ...
                {
                    this.subBlueprintMemberInfos ??= new List<CsvHelper.MemberInfo>();
